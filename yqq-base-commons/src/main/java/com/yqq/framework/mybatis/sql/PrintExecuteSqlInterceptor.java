package com.yqq.framework.mybatis.sql;
...
public class PrintExecuteSqlInterceptor implements Interceptor {

    private static Logger log = LoggerFactory.getLogger(PrintExecuteSqlInterceptor.class);
    ...
    public Object intercept(Invocation invocation) throws Throwable {
            try{
                ...
                BoundSql boundSql;
                if(args[args.length-1] instanceof BoundSql){
                    boundSql = (BoundSql )args[args.length-1];
                }else {
                    boundSql = mappedStatement.getBoundSql(parameter);  // BoundSql就是封装myBatis最终产生的sql类
                }
