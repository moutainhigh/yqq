 ...
 */
@Component
public class MallRightsServiceFactory implements ApplicationContextAware {
    private Logger logger = LoggerFactory.getLogger(MallRightsServiceFactory.class);
     ...
     * 领取失败
     */
    private static final String RIGHTS_LQSB = "LQSB";
    /**
     * 领取成功
     */
    private static final String RIGHTS_LQCG = "LQCG";
     ...
     */
    public JSONResult<String> openRights(RightsLeadReq rightsLeadReq) {
        JSONResult<String> jsonResult = new JSONResult<>();
        if (ObjectUtils.isEmpty(rightsLeadReq)) {
            jsonResult.setErrorInfo("开通权益错误,请求信息为空", RIGHTS_LQSB);
            return jsonResult;
        }
        try {
            UserInfoBean userInfoBean = rightsLeadReq.getUserInfoBean();
            if (userInfoBean == null) {
                jsonResult.setErrorInfo("开通权益类型错误,用户不允许为空", RIGHTS_LQSB);
                return jsonResult;
            }
            String mobile = userInfoBean.getMobile();
            if (StringUtils.isEmpty(mobile)) {
                jsonResult.setErrorInfo("开通权益类型错误,用户手机号不允许为空", RIGHTS_LQSB);
                return jsonResult;
            }
            String rightsNum = rightsLeadReq.getRightsNum();
            if (StringUtils.isEmpty(rightsNum)) {
                logger.warn("{}========================开通权益类型错误,joinNum 为空", mobile);
                jsonResult.setErrorInfo("开通权益类型错误,joinNum 为空", RIGHTS_LQSB);
                return jsonResult;
            }
            // 权益接入码
            ...
            if (StringUtils.isEmpty(joinNum)) {
                ...
                jsonResult.setErrorInfo("权益无对应接入码无法开通", RIGHTS_LQSB);
                return jsonResult;
            }
            ...
            if (typeEnum.equals(OpenRightsTypeEnum.R_NULL)) {
                ...
                jsonResult.setErrorInfo("接口不支持该权益", RIGHTS_LQSB);
                return jsonResult;
            }
            // 查询权益参数
            ...
            if (rightsParams.isEmpty() && !typeEnum.equals(OpenRightsTypeEnum.R_MT)) {
                ...
                jsonResult.setErrorInfo("开通权益参数配置错误", RIGHTS_LQSB);
                return jsonResult;
            }
            OpenRightsService openRightsService = this.getOpenRightsService(typeEnum);
            RightsHandleResult handleResult = openRightsService.openRights(rightsLeadReq, rightsParams);
            String handleStatus = handleResult.getHandleStatus();
            String msg = handleResult.getHandleMsg();
            switch (handleStatus) {
                case RightsHandleResult.SUCCESS:
                    jsonResult.setCode(RIGHTS_LQCG);
                    jsonResult.setSuccessInfo(msg, "开通成功");
                    return jsonResult;
                case RIGHTS_LQSB:
                    jsonResult.setErrorInfo(msg, RIGHTS_LQSB);
                    return jsonResult;
                default:
                    jsonResult.setErrorInfo(msg, RIGHTS_LQSB);
                    return jsonResult;
            ...
            jsonResult.setErrorInfo("开通异常", RIGHTS_LQSB);
            return jsonResult;
        }
    }
